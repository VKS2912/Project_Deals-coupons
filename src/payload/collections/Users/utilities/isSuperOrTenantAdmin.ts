...
export const isSuperOrTenantAdmin = async (args: { req: PayloadRequest }): Promise<boolean> => {
  ...
  if (logs) {
    const msg = `Found tenant: '${foundTenants.docs?.[0]?.storename}', checking if user is an tenant admin`
    payload.logger.info({ msg })
  }
  ...
  if (tenantWithUser?.roles?.some(role => role === 'admin')) {
    if (logs) {
      const msg = `User is an admin of ${foundTenants.docs[0].storename}, allowing access`
      payload.logger.info({ msg })
    }
  ...
  if (logs) {
    const msg = `User is not an admin of ${foundTenants.docs[0].storename}, denying access`
    payload.logger.info({ msg })
  }
